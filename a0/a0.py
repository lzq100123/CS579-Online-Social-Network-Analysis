...
def add_all_friends(twitter, users):
    ...
    ###TODO
    for u in users:
        follows = get_friends(twitter,u['screen_name'])
        u['friends'] = follows
    return
...
def print_num_friends(users):
    ...
    """
    ###TODO
    tmp = sorted(users,key = lambda x : x['screen_name'])
    for t in tmp:
        print('%s  %d'% (t['screen_name'],len(t['friends'])))
...
def friend_overlap(users):
    ...
    for i in range(len(users)):
        for j in range(len(users))[i + 1:]:
            tl = sorted([users[i]['screen_name'],users[j]['screen_name']])
            tmp = (tl[0],tl[1],len([friend for friend in users[i]['friends'] if friend in users[j]['friends']]))
            ls.append(tmp)
    overlap = sorted(ls,key = lambda x : x[2],reverse = True)
    return overlap
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    """
    ###TODO
    Donald = [u for u in users if u['screen_name'] == 'realDonaldTrump']
    Hillary = [u for u in users if u['screen_name'] == 'HillaryClinton']
    id = [i for i in Donald[0]['friends'] if i in Hillary[0]['friends']]
    request = robust_request(twitter,'users/lookup',{'user_id': id})
    return [r for r in request][0]['screen_name']


# t = get_twitter()
# n = read_screen_names('candidates.txt')
# u = get_users(t,n)
# add_all_friends(t,u)
# print_num_friends(u)
# c = count_friends(u)
# # followed_by_hillary_and_donald(u,t)
