...
def tokenize(doc, keep_internal_punct=False):
    ...
    if not doc:
    	return np.array([])
    elif keep_internal_punct:
    	res = doc.lower().split()
    	newlist = []
    	rng = list(range(ord('a'),ord('z') + 1)) + list(range(ord('0'),ord('9') + 1))
    	...
    	for i in range(len(newlist)):
    		...
    		while ord(newlist[i][start]) not in rng or ord(newlist[i][end - 1]) not in rng:
    			if ord(newlist[i][start]) not in rng:
    				start += 1
    			if ord(newlist[i][end - 1]) not in rng:
    				end -= 1
    		newlist[i] = newlist[i][start : end]
...
def token_pair_features(tokens, feats, k=3):
    ...
    for i in range(len(tokens)):
    	if i + k > len(tokens):
    		break
    	l += list(combinations(tokens[i : i + k],2))
    for i in [t[0] + '__' + t[1] for t in l]:
    	if prefix + i in feats:
    		feats[prefix + i] += 1
    	else:
    		feats[prefix + i] = 1
...
def lexicon_features(tokens, feats):
    ...
    """
    ###TODO
    if 'neg_words' not in feats: 
    	feats['neg_words'] = 0
    if 'pos_words' not in feats: 
    	feats['pos_words'] = 0
    for i in tokens:
...
def vectorize(tokens_list, feature_fns, min_freq, vocab=None):
    ...
    if not vocab:
    	ls = sorted(list(set(ls)),key = lambda x : x[0])
    	for i in ls:
    		if i[0] not in voc:
    			voc[i[0]]
    else:
    ...
    for feats in feats_list:
    	for f in feats:
    		if f[0] in voc and f[1] >= min_freq:
    			data.append(f[1])
    			col.append(voc[f[0]])
    			row.append(index)
    	index += 1
    if len(data) == 0:
    	return None, None
    X = csr_matrix((data,(row,col)),shape=(len(feats_list), len(voc)),dtype = np.uint64)
    return X, voc
...
def cross_validation_accuracy(clf, X, labels, k):
    ...
    Returns:
      The average testing accuracy of the classifiers
...
def eval_all_combinations(docs, labels, punct_vals,
    ...
    for i in range(1, len(feature_fns) + 1):
    		for t in [list(fns) for fns in combinations(feature_fns,i)]:
    			fnset.append(t)
    for punct in punct_vals:
    	tokens_list = [tokenize(d,punct) for d in docs]
    	for min_f in min_freqs:
    		for fns in fnset:
    			X, vocab = vectorize(tokens_list, fns, min_f)
    			if X is not None:
    				clf = LogisticRegression()
    				avg = cross_validation_accuracy(clf, X, labels, 5)
    				res.append({'features': fns, 'punct': punct, 'accuracy':avg,'min_freq': min_f})
...
def plot_sorted_accuracies(results):
    ...
    """
    ###TODO
    axisx = list(range(len(results)))
    axisy = sorted([res['accuracy'] for res in results])
    ...
    for i in fnset:
    	...
    	for j in range(len(name)):
    		if j == len(name) - 1:
    			...
    			string += name[j] + ' '
    	res.append((np.mean([t['accuracy'] for t in results if t['features'] == i]),'features='+string))
    for i in min_freqs:
    	res.append((np.mean([t['accuracy'] for t in results if t['min_freq'] == i]),'min_freq='+str(i)))
    for i in punct_vals:
    	res.append((np.mean([t['accuracy'] for t in results if t['punct'] == i]),'punct='+str(i)))
...
def print_top_misclassified(test_docs, test_labels, X_test, clf, n):
    ...
    ###TODO
    res = []
    for test_idx in range(len(X_test.toarray())):
    	predict = clf.predict(X_test[test_idx])
    	if predict != test_labels[test_idx]:
    		if test_labels[test_idx]:
    			prob = clf.predict_proba(X_test[test_idx])[0][0]
    		else:
    			prob = clf.predict_proba(X_test[test_idx])[0][0]
    		res.append({'truth': test_labels[test_idx],'predict':predict,'proba':prob,'test_doc':test_docs[test_idx]})
    for r in sorted(res,key = lambda x : -x['proba'])[:n]:
    	print('truth=%d predict=%d proba=%f' %(r['truth'],r['predict'],r['proba']))
    	print(r['test_doc'])
    	print('')
