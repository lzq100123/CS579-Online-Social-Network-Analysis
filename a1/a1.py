...
def bfs(graph, root, max_depth):
    ...
    """
    ###TODO
    parents, num_paths, distances, queue = defaultdict(list), defaultdict(int), defaultdict(int), deque([root])
    num_paths[root] = 1
    distances[root] = 0
    seen = set()
    while queue:
      cur = queue.popleft()
      if  distances[cur] >= max_depth: break
      if cur not in seen:
        seen.add(cur)
      for v in graph.neighbors(cur):
        if v not in seen:
          queue.append(v)
          seen.add(v)
          parents[v].append(cur)
          num_paths[v] = num_paths[cur]
          distances[v] = distances[cur] + 1
        elif distances[cur] < distances[v] and cur not in parents[v]:
          parents[v].append(cur)
          num_paths[v] += num_paths[cur]
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    d = sorted(node2distances.items(), key = lambda x : x[1], reverse = True)
    tmp = {}
    ...
    for n in d:
      if n[0] != root:
        for p in node2parents[n[0]]:
          value = tmp[n[0]] / node2num_paths[n[0]]
          res[(min(n[0],p),max(n[0],p))] = value
          if p in tmp:
            tmp[p] += value
    return res
...
def approximate_betweenness(graph, max_depth):
    ...
    for root in graph.nodes():
      ds, num, parents = bfs(graph,root,max_depth)
      edges = bottom_up(root,ds,num,parents)
      for e in edges:
        if e not in res:
          res[e] = edges[e]
        else:
...
def partition_girvan_newman(graph, max_depth):
    ...
    ###TODO
    gcp = graph.copy()
    eb = approximate_betweenness(gcp,max_depth)
    ec = sorted(eb.items(), key = lambda x : (-x[1],x[0][0]))
    components = [c for c in nx.connected_component_subgraphs(gcp)]
    while(len(components) == 1):
      gcp.remove_edge(*ec[0][0])
      ec.remove(ec[0])
      components = [c for c in nx.connected_component_subgraphs(gcp)]
    return components
...
def score_max_depths(graph, max_depths):
    ...
    for depth in max_depths:
      ...
      res.append((depth,norm_cut(components[0].nodes(),components[1].nodes(),graph)))
    return res
...
def make_training_graph(graph, test_node, n):
    ...
    ###TODO
    ng = graph.copy()
    neighbors = sorted(ng.neighbors(test_node))
    for i in range(n):
      if i >= len(neighbors): break
      ng.remove_edge(test_node,neighbors[i])
    return ng
...
def jaccard(graph, node, k):
    ...
    tmp = []
    for n in graph.nodes():
      if n != node and (node,n) not in graph.edges():
        neighbors2 = graph.neighbors(n)
        tmp.append(((node,n),len(set(neighbors) & set(neighbors2)) / len(set(neighbors) | set(neighbors2))))
    return sorted(tmp, key = lambda x : (-x[1],x[0][1]))[: k]
...
def path_score(graph, root, k, beta):
    ...
    """
    ###TODO
    ds, num ,parents = bfs(graph,root,len(graph.nodes()))
    tmp = []
    for n in graph.nodes():
      if n != root and (root, n) not in graph.edges() and (n,root) not in graph.edges():
        tmp.append(((root, n), beta ** ds[n]*num[n]))
    return sorted(tmp, key = lambda x : (-x[1],x[0][1]))[: k]
...
def evaluate(predicted_edges, graph):
    ...
    for e in predicted_edges:
      if (e[0],e[1]) in graph.edges() or (e[1],e[0]) in graph.edges():
         count += 1
